...
import com.devmello.music.util.render.utilities.AlphaOverride;
...
public class RenderEngine {
    ...
    private static void renderRoundedQuadInternal(Matrix4f matrix, float cr, float cg, float cb, float ca, float fromX, float fromY, float toX, float toY, float radC1, float radC2, float radC3,
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (float r = i * 90f; r <= (i + 1) * 90f; r += 90 / samples) {
                ...
                bufferBuilder.vertex(matrix, current[0] + sin, current[1] + cos, 0.0F).color(cr, cg, cb, ca);
            }
        }
    ...
    public static void drawRound(MatrixStack matrices, float x, float y, float width, float height, float radius, Color color) {
        renderRoundedQuad(matrices, color, x, y, (double) width+x, (double) height + y, (double) radius, 4);
    }
    ...
    public static void renderRoundedQuadInternal(Matrix4f matrix, float cr, float cg, float cb, float ca, double fromX, double fromY, double toX, double toY, double radius, double samples) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            float rad1 = (float) java.lang.Math.toRadians((360 / 4d + i * 90d));
            float sin = (float) (java.lang.Math.sin(rad1) * rad);
            float cos = (float) (java.lang.Math.cos(rad1) * rad);
            bufferBuilder.vertex(matrix, (float) current[0] + sin, (float) current[1] + cos, 0.0F).color(cr, cg, cb, ca);
        }
    ...
    private static void renderRoundedOutlineInternal(Matrix4f matrix, float cr, float cg, float cb, float ca, float fromX, float fromY, float toX, float toY, float radC1, float radC2, float radC3,
        ...
        float rad = current[2];
        bufferBuilder.vertex(matrix, current[0], current[1] + rad, 0.0F).color(cr, cg, cb, ca);
        bufferBuilder.vertex(matrix, current[0], current[1] + rad + width, 0.0F).color(cr, cg, cb, ca);
        BufferUtils.draw(bufferBuilder);
    }
